...
public class CategoriesController : ControllerBase {
    ...
    public async Task<ActionResult<ApiResponse<Category>>> GetById(int id) {
        ...
        if (category is null) return NotFound(ApiError.NotFound($"Категория с id={id} не найдена"));
    ...
    public async Task<ActionResult> GetWithNotes(int id) {
        ...
        if (category is null) return NotFound($"Категория с id = {id} не найдена");
    ...
    public async Task<ActionResult> Delete(int id) {
        ...
        if (await _repo.HasNotesAsync(id)) {
            return BadRequest(ApiError.BadRequest("Невозможно удалить категорию: в нём есть заметки. " + "Сначала удалите или переместите заметки."));
        }
